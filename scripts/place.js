...
/** * Calculates the Wind Chill factor in Celsius. * * Formula: Wind Chill (°C) = 13.12 + 0.6215*T - 11.37*V^0.16 + 0.3965*T*V^0.16 * Standard: Environment Canada / ISO Meteorological Standard * *
 // @//param {number} temperature  - Air temperature in degrees Celsius (T) * @p//aram {number} windSpeed    - Wind speed in km/h (V), must be > 4.8 km/h * @returns {number}            - Wind chill temperature in °C (rounded to 2 decimals) * * NOTE: This formula is designed for temperatures <= 10°C and wind speeds > 4.8 km/h. * At higher temps (like DRC's 33°C), it still computes but the "feels like" model * for hot weather is the Heat Index formula instead. */

function calculateWindChill(temperature, windSpeed) {return parseFloat((13.12 + 0.6215 * temperature - 11.37 * Math.pow(windSpeed, 0.16) + 0.3965 * temperature * Math.pow(windSpeed, 0.16)).toFixed(2));}
